...
if cells == 0:
    if 'IV' in Cell['ExptID']:
        ...
        for i in Cell['Data'][x:Cell['TraceMax'][1]:1]:
            ...
            plt.plot(Cell['Time'][0:20000],Cell['Data'][:,x][0:20000])
            ...
            Trace_Results = {'AP #': int(traces_results[0]['Spikecount']), 'RMP': float(traces_results[0]['voltage_base']),
                             ...
                             'Mean_Half-Height_Width': np.mean(traces_results[0]['spike_width2']),
...
#Calculates IV curve parameters FOR TWO CELLS including spikecount, RMP, Rin, Freq, etc.
    ...
    for ii in range(cells):
        if 'IV' in Cell['ExptID']:
            User_choices['IV_graph' + str(ii + 1)] = int(input('Would you like to save the graph? (Input 1): '))
            User_choices['IV_output' + str(ii + 1)] = int(input('Would you like to output the results as a CSV? (Input 1): '))
            ...
            for i in Cell['C'+ str(ii + 1) + 'Data'][x:Cell['TraceMax'][1]:1]:
                ...
                #Plot individual sweeps in the output window
                plt.plot(Cell['Time'][0:20000],Cell['C' + str(ii + 1) + 'Data'][0:20000,x])
                plt.title('Trace ' + str(x))
                plt.scatter(traces_results[0]['peak_time'],traces_results[0]['peak_voltage'] + 20, c = 'r', marker = '+')
                plt.xlabel('Time (in ms)')
                plt.ylabel('Voltage (in mV)')
            ...
            plt.plot(Cell['Time'][0:20000],Cell['C' + str(ii + 1) + 'Data'][0:20000])
            plt.title('All Traces')
            plt.xlabel('Time (in ms)')
            plt.ylabel('Voltage (in mV)')
...
if cells == 0:
    if 'SP CC' in Cell['ExptID']:
        ...
        User_choices['SPCCGraph'] = input('Would you like to save the average/individual trace graph? (Input 1): ')
...
if cells == 2:
    for ii in range(cells):
        if 'SP CC' in Cell['ExptID']:
            #Allows the user to save the average graph, if needed
            User_choices['SPCCGraph' + str(ii + 1)] = input('Would you like to save the average/individual trace graph? (Input 1): ')
...
if cells == 0:
    if 'PPR CC' in Cell['ExptID']:
        #Allows the user to save the average graph, if needed
        User_choices['PPRGraph'] = input('Would you like to save the average/individual trace graph? (Input 1: ')
...
if cells == 0:
    if 'Train CC' in Cell['ExptID']:
        #Allows the user to save the average graph, if needed
        User_choices['TrainGraph'] = input('Would you like to save the average/individual trace graph? (Input 1): ')
...
if cells == 0:
    if 'SP E/I' in Cell['ExptID']:
        #Allows the user to save the average graph, if needed
        User_choices['EIGraph'] = input('Would you like to save the average/individual trace graph? (Input 1): ')
        ...
        fig,ax = plt.subplots()
        for i in range(Cell['TraceMax'][1]):
            ax.plot(Cell['Time'][9500:12000],EI_zeroed[i][9500:12000], c = 'lightgray', linewidth = 0.3)
            ax.plot(Cell['Time'][9500:12000],AvgTrace[9500:12000],c = 'k')
        ax.scatter(500,-2,c = 'blue', marker = 's')
        ax.title.set_text('Average PSP')
        ax.set_xlabel('Time (in ms)')
        ax.set_ylabel('PSP Amplitude (in mV)')
        if User_choices['EIGraph'] == 1:
            plt.savefig('SPEI.svg',format = 'svg',dpi = 1200)
        plt.show()
        plt.close()
