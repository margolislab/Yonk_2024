            ...
            for jj in range(traces_results[0]['Spikecount'][0]):
                ...
                for ii in range(len(HHWData[TUpstart:TPeak])):
                    if HHWData[TUpstart + ii] >= thres:
                        UpP1 = HHWData[TUpstart + ii]
                        UpM1 = HHWData[TUpstart + (ii - 1)]
                        if abs(UpP1 - thres) > abs(UpM1 - thres):
                            HHWU = Cell['Time'][TUpstart + (ii - 1)]
                        elif abs(UpP1 - thres) < abs(UpM1 - thres):
                            HHWU = Cell['Time'][TUpstart + ii]
                        break
                for iii in range(len(HHWData[TPeak:TDownstart])):
                    if HHWData[TPeak + iii] <= thres:
                        DP1 = HHWData[TPeak + iii]
                        DM1 = HHWData[TPeak + (iii - 1)]
                        if abs(DP1 - thres) > abs(DM1 - thres):
                            HHWD = Cell['Time'][TPeak + (iii - 1)]
                        elif abs(DP1 - thres) < abs(DM1 - thres):
                            HHWD = Cell['Time'][TPeak + iii]
                        break
                AllHHWU.append(HHWU)
                AllHHWD.append(HHWD)
