...
if 'IV' in Cell['ExptID']:
    ...
    for i in range(i,Cell['TraceMax'][1],1):
        ...
        HHWApp = [x for x in HHWApp if x > 0] #This removes any valueless items within the HHWApp
...
if 'SP CC' in Cell['ExptID']:
    ...
    for i in range(Cell['TraceMax'][1] - User_choices['SP_NaN_Trace']): #19900:21000
        ...
        ax.scatter(1000,-2,c = 'blue', marker = 's')
...
if 'PPR CC' in Cell['ExptID']:
    ...
    if User_choices['PPR NaN'] == 0:
        print('Continuing to PPR Analysis')
    if User_choices['PPR NaN'] == 1:
        ...
        for i in range(User_choices['PPR_NaN_Trace']):
            ...
            Cell['Data'] = np.delete(Cell['Data'],User_choices['PPR_Traces'],axis = 1)
            Cell['TraceMax'] = Cell['TraceMax'] - User_choices['PPR_NaN_Trace']
    ...
    #Calculate max amplitude and latency for all 5 peaks compared to single baseline value for each sweep
    PPRAmp = np.transpose(PPRAmp)
    for i in range(int(Cell['TraceMax'])):
        iterstart = 20001
        iterend = 21000
        for ii in range(5):
            psp = np.nanmax(PPRAmp[iterstart:iterend,i])
            lat = np.nanargmax(PPRAmp[iterstart:iterend,i])
            PPRPSP.append(psp)
            PPRLatAdd.append(lat + iterstart)
            PPRLat.append(lat)
            iterstart += 1000
            iterend += 1000

    #Define exponential function for more accurate amplitudes for pulses 2-4
    def func(x, a, b, c):
        return a*np.exp(-b*x)+c
    
    count = 0 #Set counter to iterate through the PPRLatAdd list correctly    
    #Capture optimal fitting parameters of the curve including fitting parameters (popt) and approximate covariance of popt (pcov)
    for j in range(int(Cell['TraceMax'])):
        for jj in range(5):
            ydata = PPRAmp[PPRLatAdd[count]:PPRLatAdd[count]+800,j] #Capture the ydata from the maximal amplitude point + 700 samples
            xdata = range(0,np.size(ydata)) #Match the xrange size to the ydata size
                
            popt, pcov = curve_fit(func,xdata,ydata, maxfev = 5000) #Fit the exponential line to the curve
            p1 = popt[0] #Capture a from exponential function
            p2 = popt[1] #Capture b from exponential function
            p3 = popt[2] #Capture c from exponential function
                
            curvey = func(xdata,p1,p2,p3) #Create the trendline
            plt.plot(curvey,'r--')
            plt.plot(ydata,'g')
            plt.show() #Show plot of fitted line with decay of each PSP
            
            f = interpolate.interp1d(xdata,curvey, fill_value = 'extrapolate')
            Extrap = list() #Create a list to save extrapolated values
            for iii in range(400):
                extrap_values = f(800 + iii) #Using fitted function, calculate extrapolated values for each sampling point
                Extrap.append(extrap_values)
            
            curvey = np.concatenate((curvey,Extrap)) #Concatenate fitted and extrapolated values together
            new_range = list(range(PPRLatAdd[count],PPRLatAdd[count]+1200)) #Create a new xrange to incorporate all data points
            new_ydata = PPRAmp[new_range,j]
            
            plt.plot(curvey) 
            plt.plot(new_ydata) #Plot the trendline overlaid on the actual PSP decay
            plt.show()       
            
            if jj == 4: #Skips over the last pulse on every 5th iteration
                print('No following PSP')
                count += 1
                continue
            
            if jj == 0: 
                PPRTrueAmp.append(PPRPSP[count]) #Condition to add the initial PSP into the PPRTrueAmp list
            
            PAmp = np.nanmax(new_ydata[600:1200]) #Calculates the maximal amplitude on the following PSP
            PLat = np.argmax(new_ydata[600:1200]) + 600 #Calculates the index of the max amplitude of the following PSP
            print('The maximal amplitude of the following PSP is: ' + str(PAmp))
            print('The index of the max amp of the following PSP is ' + str(PLat))
            print('The y-value of the fitted curve is ' + str(f(PLat)))
            
            AMP = PAmp - f(PLat) #Subtracts the maximal PSP amplitude from the fitted exponential curve
            print('The amplitude of the pulse is ' + str(AMP))
                
            PPRTrueAmp.append(AMP)
            count += 1 #Iterate through the PPRLat list
            
            
            
            
            
            ydata = PPRAmp[PPRLatAdd[95]:PPRLatAdd[95]+800,19]
            xdata = range(0,np.size(ydata))
            
            popt, pcov = curve_fit(func,xdata,ydata,maxfev = 5000)
            p1 = popt[0]
            p2 = popt[1]
            p3 = popt[2]
            curvey = func(xdata,p1,p2,p3)
            plt.plot(curvey,'r--')
            plt.plot(ydata,'g')
            plt.show()
            plt.savefig('trend.svg',format = 'svg',dpi =1200)
    ...
    PPRTrueAmp = np.array(PPRTrueAmp)
    PPRTrueAmp = np.split(PPRTrueAmp,int(Cell['TraceMax']),0)
    PPRTrueAmp = np.column_stack(PPRTrueAmp)

    #Calculate the average PSP for each pulse
    for i in range(5):
        MeanPSP = np.nanmean(PPRTrueAmp[i])
    ...
    for i in range(Cell['TraceMax'] - User_choices['PPR_NaN_Trace']):
