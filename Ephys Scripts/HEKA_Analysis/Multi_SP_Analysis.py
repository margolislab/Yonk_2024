...
for key,values in Datadict.items():
    ...
    Max = pd.Series.max(Avg.iloc[9900:10500])
    Meandict[key] = Avg
    Maxdict[key] = Max
...
AvgTrace = pd.DataFrame.mean(AvgTrace,axis = 0)
...
MaxPeak = (np.argmax(AvgTrace.loc[99000:10500]) + 10000)
Rise = (np.argmax(AvgTrace.loc[9900:10500] > BaseStdDev) + 10000)
...
plt.xlim([0,2])
if SaveGraph == 0:
    plt.savefig('SPBar.svg',format = 'svg',dpi = 1200)
plt.show()
plt.close()
...
plt.figure()
plt.plot(AvgTrace[9900:10500], c ='k',linewidth = 5)
for i in range(CellNum):
    plt.plot(Meandict[i][9900:10500],c = next(cycol2))
