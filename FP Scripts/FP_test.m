...
Last modification date: 2023-11-08
...
%analysis
aurocpath = 'C:\Users\ajy31\OneDrive - Rutgers University\Documents\a. Scripts\AUC';
...
%Permits user to select Pupil folder
disp('Please select pupil folder');
[pfile,PupilPath] = uigetfile('*.csv');
Pup = readtable(pfile,'NumHeaderLines',1);
Pup = table2array(Pup);
...
%Resampling and Rational Fraction Approximation to align Ca2+ and Video
%signals
D2 = Pup;
SampFreq = length(Z)/1000;
SampFreq2 = length(D2)/1000;
[AA,BB] = rat(SampFreq/SampFreq2);
AllPupil = resample(D2,AA,BB);
AllPupil = AllPupil(1:end-150);
minPupil = min(AllPupil);
maxPupil = max(AllPupil);

for i = 1:length(AllPupil)
    NormPupil(i) = ((AllPupil(i)- minPupil)/(maxPupil - minPupil))*100;
end
...
%For the length of the TDMS window trial, capture the FP and pupil data within the
%same window
PTend = round(2*Fs);
Grace = round(.5*Fs);
for i = 1:150
    for ii = 1:length(All{i,7})
        ...
        tempAlignPup = double(NormPupil(key-before:key+after));
        All{i,8} = tempAlign;
        All{i,13} = tempAlignPup;
...
indexgo = 1; %sets up index to calculate total number of Go trials (Hits + Misses)
indexnogo = 1; %sets up index to calculate total number of NoGo trials (FAs + CRs)
...
for i = 1:150
    if strcmp(All{i,1}, 'Hit') %If trial is Hit, store relevant variables in cell array
        ...
        h{indexh,7} = All{i,13}; %Pupil-related dynamics

        go{indexgo,1} = All{i,4}; %Lick-related voltages
        go{indexgo,2} = All{i,7}; %Voltage-related time in seconds
        go{indexgo,3} = All{i,8}; %Calcium signals
        go{indexgo,4} = All{i,12}; %Number and index of licks relative to voltage
        go{indexgo,5} = All{i,5}; %Index of presentation time window start
        go{indexgo,6} = All{i,9}; %Index of first lick outside of the grace period
        go{indexgo,7} = All{i,13}; %Pupil-related dynamics
    ...
    elseif strcmp(All{i,1}, 'Miss') %If trial is Miss, store relevant variables in cell array
        ...
        m{indexm,7} = All{i,13}; %Pupil-related dynamics
    ...
    elseif strcmp(All{i,1}, 'False Alarm') %If trial is FA, store relevant variables in cell array
        ...
        f{indexf,7} = All{i,13}; %Pupil-related dynamics

        nogo{indexnogo,1} = All{i,4}; %Lick-related voltages
        nogo{indexnogo,2} = All{i,7}; %Voltage-related time in seconds
        nogo{indexnogo,3} = All{i,8}; %Calcium signals
        nogo{indexnogo, 4} = All{i,12}; %Number and index of licks relative to voltage
        nogo{indexnogo,5} = All{i,5}; %Index of presentation time window start
        nogo{indexnogo,6} = All{i,10}; %Index of first lick outside of the grace period
        nogo{indexnogo,7} = All{i,13}; %Pupil-related dynamics
        
        indexf = indexf + 1;
        indexnogo = indexnogo + 1;
    ...
    elseif strcmp(All{i,1}, 'Correct Rejection') %If trial is CR, store relevant variables in cell array
        ...
        no{indexno,7} = All{i,13}; %Pupil-related dynamics
    ...
    elseif strcmp(All{i,1}, 'Auto Reward') %If trial is AR, store relevant variables in cell array
        ...
        ar{indexar,7} = All{i,13}; %Pupil-related dynamics
        indexar = indexar + 1;
    end
    ...
    if i == 150
        ...
        indexgo = indexgo - 1; %As each iterating variable starts at 1, subtract one at the end
        indexnogo = indexnogo - 1; %As each iterating variable starts at 1, subtract one at the end
    end
end
...
%Lick rate relative to texture presentation (2s pre and post)
if NoGoTest == 1
    temp = LPTend - Starth;
    t2 = Starth - temp; %This variable marks 2s before texture presentation
...
if Reward == 1
    ...
    if indexar > 0
        for ii = 1:indexar
            [ConPksar,ConLocsar] = findpeaks(ARGCaMP(:,ii),'MinPeakHeight',prctile(ARGCaMP(:,ii),90),'MinPeakWidth',2); %Find peaks within the trial window that are at or above the 90th percentile
            ConCaAR{ii} = [ConPksar ConLocsar]; %Save the amplitude and corresponding index into a new variable
            ConCaARtemp = [0 0]; %(Re)set the temporary variable in case multiple peaks are detected within the window
    
            for iii = 1:length(ConCaAR{1,ii}) %Iterate through the list of peaks detected during a trial
                if ConCaAR{1,ii}(iii,2) <= temp2 && ConCaAR{1,ii}(iii,2) >= 0
                    ConCaARtemp(iii,1:2) = ConCaAR{1,ii}(iii,1:2); %Capture the Ca2+ signals and timepoint if they are within +/- 2s window of the texture presentation
                end
            end
        [~,ConCaARtempMax] = max(ConCaARtemp(:,1)); %Take the maximal Ca2+ signal value within the +/- 2s window
        ConCaARAmp(ii,1:2) = ConCaARtemp(ConCaARtempMax,:); %Append maximal Ca2+ signal and timepoint value into a single array
        end
    MeanConCaARAmp = mean(ConCaARAmp(:,1));
    MeanConCaARLatency = mean(ConCaARAmp(:,2));
    elseif indexar == 0
        ConCaARAmp = 0;
        MeanConCaARAmp = 0;
        MeanConCaARLatency = 0;
    end
end

%% Lick Analysis
AllPupil = padcat(All{:,13});
...
if indexnogo > 0
    NoGoPupil = padcat(nogo{:,7});
end

%% AUROC Analysis
AllAUROC = padcat(All{:,8})';
AllAUROCTrunc = AllAUROC(:,1:6000);
SlidingSteps = 20;
WindowSize = 100;
StartValue = 1000;
EndValue = 5800;
...
for yy = StartValue:SlidingSteps:EndValue
    ...
    yy
    [Allm Alln] = size(AllAUROCTrunc);
    for x = 1:Allm
        y(x,1) = mean(AllAUROCTrunc(x,InitialBasalWindow:EndBasalWindow));
        y2(x,1) = mean(AllAUROCTrunc(x,yy:yy + WindowSize));
    end
    y3 = cat(1,y,y2);
...
figure
plot(WinTime,AreaUCAll)
title('All AUROC')
[AllAUROCAmp,AllAUROCind] = max(AreaUCAll);
AllAUROCind = WinTime(AllAUROCind);
...
if indexgo > 1
    AllZGoT = padcat(go{:,3})'; %Combine all ZMAD calcium signals for NoGo trials
    AllZGoTTrunc = AllZGoT(:,1:6000);
    SlidingSteps = 20; %Time value of sliding window
    WindowSize = 100;
    StartValue = 1000;
    EndValue = 5800;
    ...
    for yy = StartValue:SlidingSteps:EndValue
        ...
        yy
        [nom non] = size(AllZGoTTrunc);
        for x = 1:nom
            y(x,1) = mean(AllZGoTTrunc(x,InitialBasalWindow:EndBasalWindow));
            y2(x,1) = mean(AllZGoTTrunc(x,yy:yy+WindowSize));
        end
    ...
    figure
    plot(WinTime,AreaUCGoT)
    title('All Go AUROC')
    [GoTAUROCamp,GoTAUROCind] = max(AreaUCGoT);
    GoTAUROCind = WinTime(GoTAUROCind);
end

if indexnogo > 0
    AllZNoGoT = padcat(nogo{:,3})'; %Combine all ZMAD calcium signals for NoGo trials
    AllZNoGoTTrunc = AllZNoGoT(:,1:6000);
    SlidingSteps = 20; %Time value of sliding window
    WindowSize = 100;
    StartValue = 1000;
    EndValue = 5800;
    ...
    for yy = StartValue:SlidingSteps:EndValue
        ...
        yy
        [nom non] = size(AllZNoGoTTrunc);
        for x = 1:nom
            y(x,1) = mean(AllZNoGoTTrunc(x,InitialBasalWindow:EndBasalWindow));
            y2(x,1) = mean(AllZNoGoTTrunc(x,yy:yy+WindowSize));
        end
    ...
    figure
    plot(WinTime,AreaUCNoGoT)
    title('All NoGo AUROC')
    [NoTAUROCamp,NoTAUROCind] = max(AreaUCNoGoT);
    NoTAUROCind = WinTime(NoTAUROCind);
end

if indexar > 0
    AllZAR = padcat(ar{:,3})';
    AllZARTrunc = AllZAR(:,1:6000);
    SlidingSteps = 20;
    WindowSize = 100;
    StartValue = 1000;
    EndValue = 5800;
    ...
    for yy = StartValue:SlidingSteps:EndValue
        ...
        yy
        [arm arn] = size(AllZARTrunc);
        for x = 1:arm
            y(x,1) = mean(AllZARTrunc(x,InitialBasalWindow:EndBasalWindow));
            y2(x,1) = mean(AllZARTrunc(x,yy:yy+WindowSize));
        end
    ...
    figure
    plot(WinTime,AreaUCAR)
    title('AR AUROC')
    [ARAUROCAmp,ARAUROCind] = max(AreaUCAR);
    ARAUROCind = WinTime(ARAUROCind);
end
...
Final.AllAUROCamp = AllAUROCAmp;
Final.AllPupil = AllPupil;
...
if Shaping == 1 
    if indexh > 0
        ...
        Final.HitGCaMP = HitGCaMP;
    end

    if indexm > 1
        Final.Misses = m;
        Final.MAUROC = [MAUROCamp, MAUROCind];
        Final.RawCaMissSignal = CaMissAmp;
        Final.MissGCaMP = MissGCaMP;
    else
        Final.Misses = [];
        Final.MAUROC = [];
        Final.RawCaMissSignal = [];
        Final.MissPupil = [];
    end
    Final.MeanCaSignals = [MeanCaHitAmp, MeanCaMissAmp];
    Final.MeanCaLatency = [MeanCaHitLatency, MeanCaMissLatency];
    Final.AllPupil = AllPupil;
    Final.HitPupil = HitPupil;
...
elseif NoGoTest == 1
    ...
    if indexf > 0
        Final.FAs = f;
        Final.FAUROC = [FAUROCamp, FAUROCind];
        Final.RawCaFASignal = CaFAAmp;
        Final.FAGCaMP = FAGCaMP;
        Final.FAPupil = FAPupil;
    else
        Final.FAs = [];
        Final.FAUROC = [];
        Final.FAGCaMP = [];
    ...
    if indexno > 0
        ...
        Final.NoGoGCaMP = NoGCaMP;
        Final.NoPupil = NoPupil;
    else
        Final.NoGos = [];
        Final.NoAUROC = [];
        Final.NoPupil = [];
    ...
    if indexgo > 0
        Final.Go = go;
        Final.GoAUROC = GoTAUROCamp;
    end

    if indexnogo > 0
        Final.NoGo = nogo;
        Final.NoGoAUROC = NoTAUROCamp;
    end
...
elseif Reward == 1
    if indexar > 0
        Final.ARs = ar;
        Final.ARAUROC = [ARAUROCAmp, ARAUROCind];
        Final.RawCaARSignal = ConCaARAmp;
        Final.ARGCaMP = ARGCaMP;
    end
    ...
    Final.MeanCaSignals = [MeanConCaARAmp, MeanCaNoGoAmp];
    Final.MeanCaLatency = [MeanConCaARLatency, MeanCaNoGoLatency];
    Final.AllPupil = AllPupil;
    Final.ARPupil = ARPupil;
    Final.NoPupil = NoPupil;
